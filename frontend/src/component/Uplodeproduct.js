...
export default function Uplodeproduct({ onclose,fetchdata }) {
    ...
    const handleSubmit = async(e) => {
        ...
        const response = await fetch(summeryApi.uplodeproduct.url,{
            method:summeryApi.uplodeproduct.method,
            credentials:"include",
            headers:{
        ...
        // console.log("respose data uploa product",responsedata)
        ...
        <div className='fixed w-full bg-slate-200 h-full top-0 left-0 right-0 bottom-0 bg-opacity-35 flex justify-center items-center'>
            <div className='bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden'>
                <div className='flex justify-between items-center pb-3'>
                    <h2 className='text-xl'> Uplode Product </h2>
                ...
                <form className='grid p-4 gap-3 overflow-y-scroll h-full pb-20' onSubmit={handleSubmit}>
                    ...
                    <label htmlFor='uplodeimageinput'>
                        <div className='p-2 bg-slate-100 border rounded h-32 w-full flex justify-center items-center cursor-pointer'>
                            <div className='text-slate-500 flex justify-center items-center flex-col gap-2'>
                                <span className='text-4xl'><FaCloudUploadAlt /></span>
                                <p className='text-sm'>Uplode Product Image</p>
                                <input type="file" id='uplodeimageinput' className='hidden' onChange={handleUploadProduct} required/>
                            </div>
                        </div>
                    </label>
                    <div>
                    ...
                    <button className='px-2 py-2 mb-5 bg-red-600 hover:bg-red-700 text-white rounded' type='submit'>Uplode Product</button>
                </form>
